...
static inline ppn_t page2ppn(struct Page *page) { return page - pages + nbase; }   //页指针到页号

static inline uintptr_t page2pa(struct Page *page) {        //页号----->物理地址
    return page2ppn(page) << PGSHIFT;
}

static inline struct Page *pa2page(uintptr_t pa) {         //物理地址----->页；通过算出这个物理地址所在的页号，再去pages中取出这个页
    if (PPN(pa) >= npage) {
...
static inline void *page2kva(struct Page *page) { return KADDR(page2pa(page)); }    //页指针--->物理地址---->虚拟地址

static inline struct Page *kva2page(void *kva) { return pa2page(PADDR(kva)); }    //虚拟地址---->物理地址---->页指针
...
static inline int page_ref(struct Page *page) { return page->ref; }  //获取页的引用计数

static inline void set_page_ref(struct Page *page, int val) { page->ref = val; }  //设置页的引用计数

static inline int page_ref_inc(struct Page *page) {    //增加页的引用计数
    page->ref += 1;
    return page->ref;
}

static inline int page_ref_dec(struct Page *page) {     //减少页的引用计数
    page->ref -= 1;
    return page->ref;
}

static inline void flush_tlb() { asm volatile("sfence.vma"); }    //刷新tlb
