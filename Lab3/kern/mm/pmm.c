...
// virtual address of boot-time page directory--引导时页目录的虚拟地址
pde_t *boot_pgdir = NULL;
// physical address of boot-time page directory--引导时页目录的物理地址
uintptr_t boot_cr3;
...
struct Page *alloc_pages(size_t n) {
    ...
    while (1) {
        ...
        if (page != NULL || n > 1 || swap_init_ok == 0) break;  //分配一个页且失败且可交换时--->向下运行，会调用swap_out()
...
static void boot_map_segment(pde_t *pgdir, uintptr_t la, size_t size,
           ...
           //在操作系统内核中设置页表项，将一个虚拟地址段映射到物理地址段                 
...
// Table)&PT(Page Table)
static void *boot_alloc_page(void) {   //分配一个page用于存放页表、大页页表等等等
...
pte_t *get_pte(pde_t *pgdir, uintptr_t la, bool create) {
     ...
     */
    pde_t *pdep1 = &pgdir[PDX1(la)];   //大大页，取大大页中的第PDX1(la)项
    if (!(*pdep1 & PTE_V)) {   //如果不存在
        struct Page *page;
        if (!create || (page = alloc_page()) == NULL) {   //若设置不创建，或者无法分配则返回NULL
            return NULL;
        }
        //不存在则创建
        set_page_ref(page, 1);
        uintptr_t pa = page2pa(page);
        memset(KADDR(pa), 0, PGSIZE);////分配内存需要一个虚拟地址，而此虚拟地址则由物理地址转换得到
        *pdep1 = pte_create(page2ppn(page), PTE_U | PTE_V);
    }
    pde_t *pdep0 = &((pde_t *)KADDR(PDE_ADDR(*pdep1)))[PDX0(la)];   //pdep1是一个大大页的页表项，其指向一个大页
    //把大大页的页表项中的物理地址取出来，然后转换为虚拟地址(也就是得到了下一级页表的起始地址)，取此页表的第PDX0(la)项

//    pde_t *pdep0 = &((pde_t *)(PDE_ADDR(*pdep1)))[PDX0(la)];
    if (!(*pdep0 & PTE_V)) {   //如果不存在
    	struct Page *page;
 ...
 //   	memset(pa, 0, PGSIZE);
    ...
    return &((pte_t *)KADDR(PDE_ADDR(*pdep0)))[PTX(la)];   //返回的相当于是一个叶子节点，一个页表项的地址，但此地址内的值可能为空
}

// get_page - get related Page struct for linear address la using PDT pgdir
struct Page *get_page(pde_t *pgdir, uintptr_t la, pte_t **ptep_store) {
    ///ptep_store的作用是储存这个页表项的地址
    pte_t *ptep = get_pte(pgdir, la, 0);
    if (ptep_store != NULL) {
        *ptep_store = ptep;
    }
    if (ptep != NULL && *ptep & PTE_V) {  //如果存在
...
static inline void page_remove_pte(pde_t *pgdir, uintptr_t la, pte_t *ptep) {  //通过设置页表项为空，来释放页表项
    /*
     *
     ...
     * flags bit : Present
     */
    if (*ptep & PTE_V) {  //若存在 //(1) check if this page table entry is
...
// validated pte
void page_remove(pde_t *pgdir, uintptr_t la) {   //从页表中去除页表项
    pte_t *ptep = get_pte(pgdir, la, 0);   //得到此页表项
    if (ptep != NULL) {
...
// note: PT is changed, so the TLB need to be invalidate
int page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) {  //新增一个映射关系---在内存中的页表中插入一个页表项

   //la是线性地址，pgdir是页目录的虚拟地址，perm是权限
    pte_t *ptep = get_pte(pgdir, la, 1); //在页目录中查找线性地址对应的页表项，如果不存在则创建一个
    if (ptep == NULL) {
        return -E_NO_MEM;
    }
    page_ref_inc(page);
    if (*ptep & PTE_V) {  //若存在   //*ptep得到的是一个页表项，如果这个页表项是有效的，那么就说明这个线性地址已经被映射了
        struct Page *p = pte2page(*ptep);//这个页表项指向的页
        if (p == page) {   //如果这个页表项对应的页和要插入的页是同一页
            page_ref_dec(page);
        } else {   //如果这个页表项对应的页不是要插入的页，那么就需要解除映射关系
            page_remove_pte(pgdir, la, ptep);  //让这个页表项指向空，把原先的页换成这个新的页
        }
    }
