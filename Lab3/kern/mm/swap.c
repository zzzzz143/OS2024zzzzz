...
swap_out(struct mm_struct *mm, int n, int in_tick)
{
     ...
     for (i = 0; i != n; ++ i)
     {
          ...
          if (swapfs_write( (page->pra_vaddr/PGSIZE+1)<<8, page) != 0) {
                    ...
                    sm->map_swappable(mm, v, page, 0);  //将页面重新映射回内存
                    continue;
          }
          else {
                    cprintf("swap_out: i %d, store page in vaddr 0x%x to disk swap entry %d\n", i, v, page->pra_vaddr/PGSIZE+1);
                    *ptep = (page->pra_vaddr/PGSIZE+1)<<8;   //将页面的虚拟地址映射到交换空间的位置
                    free_page(page);
          }
          //思考： swap_in()的时候插入新的页表项之后在哪里刷新了TLB?   
          //在swap_in的时候需要调用alloc_pages()分配一个页，当没有page可供分配时alloc_pages()会调用swap_out函数，而swap_out函数会调用tlb_invalidate()函数
          //但是若页的数量仍然满足分配需求时则不会调用swap_out函数
          //但是但是，调用完swap_in后会调用page_insert()函数，该函数会调用tlb_invalidate()函数
...
swap_in(struct mm_struct *mm, uintptr_t addr, struct Page **ptr_result)
{
     ...
     ////////通过地址addr找到对应的页表项，此页表项指向的地址是交换空间的位置

     // cprintf("SWAP: load ptep %x swap entry %d to vaddr 0x%08x, page %x, No %d\n", ptep, (*ptep)>>8, addr, result, (result-pages));
    
     int r;
     if ((r = swapfs_read((*ptep), result)) != 0)    //将数据从交换空间读入到物理页面result中
     {
        assert(r!=0);
     }
     cprintf("swap_in: load disk swap entry %d with swap_page in vadr 0x%x\n", (*ptep)>>8, addr);
     *ptr_result=result;    //ptr_result指向物理页面result
     return 0;
}
...
check_content_set_SUB(struct mm_struct *mm,uintptr_t addr,int value){
     *(unsigned char *)addr = value;///访问地址addr
     sm->update_swap_cnt(check_mm_struct,addr);
}
